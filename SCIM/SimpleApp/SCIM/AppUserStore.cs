...
public class AppUserStore : IScimStore<User>
{
    ...
    //     };
    //
    // [$"{ScimSchemas.User}:emails[{ScimSchemas.User}:type eq \"work\"].{ScimSchemas.User}:value"] = (source, value) => source.Email = (string)value,
    //
    ...
    //         {
    //             if(!source.Phones.Any()) source.Phones.Add(new AppPhoneNumber());
    //             source.Phones.First().Value = (string)value;
    //         },
    //         [$"{ScimSchemas.User}:phoneNumbers[{ScimSchemas.User}:type eq \"work\"].{ScimSchemas.User}:primary"] = (source, value) =>
    //         {
    //             if(!source.Phones.Any()) source.Phones.Add(new AppPhoneNumber());
    //             source.Phones.First().Primary = (bool)value;
    //         },
    ...
    public async Task<User?> PartialUpdate(string resourceId, IEnumerable<PatchCommand> updates)
    {
        ...
        if (updates.All(pc => pc.Operation == PatchOperation.Remove))
        {
            throw new ScimStoreException("Only Replace patch command supported for Users");
        }

        foreach (PatchCommand replaceCmd in updates.Where(u => u.Operation != PatchOperation.Remove))
        {
    ...
    private static void MapPhoneNumbersToAppPhoneNumbers(User resource, AppUser user)
    {
        ...
        {
            user.Phones = resource.PhoneNumbers.Select(p => new AppPhoneNumber
            {
                Primary = p.Primary,
                Type = p.Type,
                Value = p.Value
